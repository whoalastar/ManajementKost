...
<div class="flex items-center gap-4">
    ...
    <div>
        <h1 class="text-2xl lg:text-3xl font-bold text-gray-900">Tambah Tipe Kamar</h1>
        <p class="mt-1 text-gray-500">Buat kategori tipe kamar baru</p>
    </div>
</div>
@endsection
...
@section('content')
<div class="max-w-2xl">
    <form action="{{ route('admin.room-types.store') }}" method="POST" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        @csrf
        ...
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                <textarea id="description" 
                          ...
                          placeholder="Deskripsi singkat tentang tipe kamar ini...">{{ old('description') }}</textarea>
                @error('description')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
